...
def get_numcols(dataset, emb_cols):
    non_num_cols = ['description', 'title', 'seq_description', 'seq_title'] + emb_cols
    num_cols = []
...
def deal_text_feature(dataset):
    ...
    predictors += ['title_desc_len_ratio']
...
def RNN_model(emb_cols, num_cols, dict_emb_max, vocab_size):
#def RNN_model():
...
if __name__ == "__main__":
    ...
    for i,(train_idx, test_idx) in enumerate(skf):
        ...
        hist = modelRNN.fit(X_train_final, y_train1, batch_size=batch_size, epochs=epo, 
